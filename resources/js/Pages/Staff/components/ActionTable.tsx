...
export default function ActionTable({ title, recordType, actionsData = [] }) {
    ...
    return (
        <div>
            <h4 className=" mb-2">{title} {filteredActions.length > 0 ? "(ရှိ)" : "(မရှိ)"} </h4>
            <table className="w-full border-1 border-black border-collapse  text-center">
                <thead>
                    <tr className="bg-gray-100 font-bold border-b-1 border-black">
                        <th className="border border-black p-2 w-1/4">{recordType === "court" ? "တရားစွဲခြင်းခံရသည့်ကာလ" : "အရေးယူခံရသည့် ကာလ"} </th>
                        <th className="border border-black p-2">{recordType === "court" ? "တရားစွဲခြင်းခံရသည့် အကြောင်းကိစ္စနှင့် စွဲဆိုခံရသည့် ဥပဒေပုဒ်မ" : "အရေးယူခံရသည့် အကြောင်းကိစ္စ"}  </th>
