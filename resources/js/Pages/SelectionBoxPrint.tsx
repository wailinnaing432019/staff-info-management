...
export default function SelectionBoxPrint() {
    ...
    const handleGenerateReport = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const response = await fetch(`/api/check-staff-number?staff_number=${encodeURIComponent(staffNumber)}`);
    ...
    return (
        <div >
            ...
            >
                ...
                <div>
                    <span className="text-sm font-bold block text-gray-800  group-hover:text-white ">ဝန်ထမ်းအချက်အလက် ထုတ်ယူရန်</span>
                    <span className="text-xs text-gray-400  ">Format 18 / 36 / 25 ပုံစံ Print ထုတ်ခြင်း</span>
                </div>
            </div>
                ...
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 animate-fade-in">
                    <div className="bg-white rounded-xl shadow-xl max-w-md w-full overflow-hidden border border-gray-100">
                        ...
                        <form onSubmit={handleGenerateReport} className="p-5 space-y-4">
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-700 mb-1">ဝန်ထမ်းအမှတ် (Staff Number)</label>
                                <input
                                    type="text"
                                    value={staffNumber}
                                    onChange={(e) => setStaffNumber(e.target.value)}
                                    placeholder="ဥပမာ - ST-001"
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-700 mb-1">အစီရင်ခံစာ ပုံစံ (Format)</label>
                                <select
                                ...
                                >
                                    <option value="25">Format 25 (A5 Card ပုံစံ)</option>
                            ...
                            <div className="flex justify-end gap-2 pt-2 border-t border-gray-100">
                                ...
                                >
                                    {loading ? 'စစ်ဆေးနေသည်...' : 'Print ထုတ်မည်'}
                                </button>
                            </div>
                        </form>
